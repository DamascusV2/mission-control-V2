...
export default function KnowledgePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-xs uppercase tracking-[0.3em] text-white/50">Knowledge Vault</p>
        <h2 className="text-2xl font-semibold">Mission Doctrine</h2>
      </div>
      <div className="bg-panel rounded-2xl border border-white/5 shadow-panel p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {docs.map((doc) => (
          <div key={doc.id} className="bg-panel rounded-2xl border border-white/5 shadow-panel p-5">
            <p className="text-sm font-semibold">{doc.title}</p>
            <p className="text-sm text-white/60 mt-2">{doc.summary}</p>
            <p className="text-xs text-white/40 mt-4">Updated daily</p>
          </div>
        ))}
      </div>
    </div>
